...
<body>
		...
		1. 요청명을 결정한다.
			->di/myCalculator
		2. 컨트롤러 생성한 후 해당 요청명을 매핑한 메소드 정의
			->
		...
		5. xml 설정파일을 생성한 후 <bean > 엘리먼트를 통해 빈 
				생성.		
			-> src/main/resources 하위에 Spring Bean 
				Configuration file 을 생성.
		...
		4. 컨트롤러에서 외부파일을 읽어서 프로그램에 적용
			...
			4-4) 외부파일을 가져와서 addList로 추가후 내용읽음
		...
		<li>
			<a href="environment/main1" target="blank">
				외부파일 참조하기1(Environment객체 사용)
			</a>
		</li>
		<li>
			<a href="environment/main2" target="blank">
				외부파일 참조하기2(XML설정파일 사용)
			</a>
		</li>
		<li>
			<a href="environment/main3" target="blank">
				외부파일 참조하기3(어노테이션 사용)
			</a>
		</li>
	    ...
	    5. 파일 다운로드를 위해 빈을 생성할때
	    	common.FileDownloadView 클래스가 생성되어 있어야 한다.
	    	응답헤더와 스트림을 통해 파일을 다운로드하기 위한 로직으로
	    	구성됨.
		 -->
		<h3>파일 업로드</h3>
		<li>
			<a href="./fileUpload/uploadPath.do" target="blank">
		...
		B에게는 입금이 되지 않은 상황이 발생된다면 해당 거래는 취소되어야 한다ㅣ
		-이와 같이 양쪽 모두 만족되어야 하나의 프로세스를 완료처리할 수 있도록
		해주는 기법을 트랜잭션이라 한다.
		
		-즉 2개 이상의 쿼리를 하나의 커넥션으로 묶어 DB에 전송하고
		이 과정에서 에러가 발생하는 경우 양쪽 모두를 원래의 상태로 돌려놓는다
		
		절차
		1. 트랜잭션 메니져를 사용하는 경우
		1-1 오라클 드라이버, SPRIng-JDBC에 대한 의존설정
		1-2 servlet-context.xml에서 빈을 생성
		1-3 TicketDAO의 빈을 컨트롤러에서 자동주입 받아 사용
		1-4 2개 이상의 작업을 try-catch로 묶어 하나라도 오류가 발생할 경우

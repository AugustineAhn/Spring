...
public class FileuploadController {
	...
	public void uploadPath(HttpServletRequest req,
			HttpServletResponse resp) throws IOException{//view까지 갈필요없이 void로 처리
	...
	/*
	 UUID(Universally Unique Identifier)
	 	:범용고유식별자. randomUUID() 메소드를 통해
	 	문자열을 생성하면 하이픈이 4개 포함된 32자의 랜덤하고
	 	유니크한 문자열이 생성된다.
	 	* JDK에서 기본적으로 제공되는 클래스.
	 */
	...
	public String uploadAction(Model model, MultipartHttpServletRequest req) {
		...
		try {
			...
			while(itr.hasNext()) {
				...
				//위 4갸지 정보를 저장한 Map을 ArrayList에 저장한다.
				resultList.add(file);
		...
		@RequestMapping("/fileUpload/download.go")

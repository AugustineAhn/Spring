...
public class TicketDAO {
	...
	/*
	 위 2개의 멤버변수는 servlet-context.xml에서 TransactionDAO 빈을 생성할때
	 초기화된다.
	*/
	...
	public void buyTicket(final TicketDTO dto) {
		...
		/*
		 ...
		 구매해본다. 6장 이상인 경우 pay테이블에는 insert되지만
		 ticket테이블에는 제약조건 위배로 insert 되지 않는다.
		 */
	...
	try {
		...
		/*
		 티켓 구매갯수가 5이하인 경우 정상처리되어 모든 작업이 commit.
		 */
		System.out.println("카드결재와 티켓구매 모두"
				+ " 정상처리되었습니다.");
		transactionManager.commit(status);
		}
		catch(Exception e) {
			/*
			 티켓 구매갯수가 6 이상인 경우 예외가 발생되어 모든 작업은
			 취소(rollback) 처리됨.
			 */
